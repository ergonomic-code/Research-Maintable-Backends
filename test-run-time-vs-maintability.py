...
# === Преобразуем категории в численные ранги ===
order = {
    "0-1 секунду": 1,
    "2-10 секунд": 2,
    "10-60 секунд": 3,
...
df[col_time_single] = df[col_time_single].map(order)
df = df.dropna(subset=[col_time_single])
...
# Разделяем группы
support_yes = df[df[col_support] == 1][col_time_single]
support_no = df[df[col_support] == 0][col_time_single]

# Проверка, что обе группы не пустые
if len(support_yes) == 0 or len(support_no) == 0:
    raise ValueError("Одна из групп пуста после фильтрации — проверь данные.")

# === Тест Манна–Уитни ===
stat, p = mannwhitneyu(support_yes, support_no, alternative="two-sided")

# === Результаты ===
print("=== Среднее время одного теста в цикле разработки (категориально) ===")
print(f"N = {len(df)}")
