...
col_loc = "Количество строк в кодовой базе проекта вместе с тестами"
...
plt.ylabel("Количество строк кода (лог шкала)")
plt.title("Размер кодовой базы vs Поддерживаемость")
plt.tight_layout()
plt.savefig("sloc_vs_maint_boxplot.png", dpi=300)
plt.close()
...
plt.xscale("log")
plt.xlabel("Средний размер кодовой базы (LOC, лог шкала)")
plt.ylabel("Доля поддерживаемых проектов")
plt.title("Тренд поддерживаемости в зависимости от размера кодовой базы")
plt.ylim(0, 1)
plt.grid(True, which="both", axis="both", linestyle="--", alpha=0.5)
plt.tight_layout()
plt.savefig("sloc_vs_maint_trend.png", dpi=300)
plt.close()
